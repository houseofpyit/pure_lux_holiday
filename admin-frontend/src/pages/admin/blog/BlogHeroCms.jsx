/**
 * BlogHeroCms — Blog Page › Hero Section
 *
 * Fields: label, heading, description, background_image_id, overlay_opacity, is_active
 *
 * Backend status: No dedicated blog page CMS endpoint exists.
 * Settings are stored in localStorage under key 'blog_hero_draft' until a
 * BlogPage model + GET/PUT endpoint is added to the backend.
 */
...
const DRAFT_KEY = 'blog_hero_draft';
...
const DEFAULTS = {
  ...
  description: 'Discover expert travel guides, destination stories, and insider tips from our luxury travel specialists.',
...
export default function BlogHeroCms() {
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Hero Section"
        description="Top banner of the Blog landing page"
      ...
      <div className="flex items-start gap-3 mb-5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
        ...
        <span>
          ...
          <code className="text-xs bg-amber-100 px-1 rounded">BlogPage</code> model is added to the backend.
        </span>
      </div>
        ...
        <div className="mb-6 rounded-xl overflow-hidden border border-border">
          <div className="relative h-48 flex items-center justify-center overflow-hidden">
            ...
            <div
              ...
              style={{ opacity: (form.overlay_opacity ?? 55) / 100 }}
            />
            <div className="relative z-10 text-center px-6">
              {form.label && (
                <p className="text-[10px] font-semibold uppercase tracking-widest text-primary/80 mb-2">
                  {form.label}
                </p>
              )}
      ...
      <div className="bg-white border border-border rounded-xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          {activeTab === 'Content' && (
            <>
              <DrawerField label="Section Label" hint='Eyebrow text above the heading (e.g. "Travel Journal")'>
                <DrawerInput
                  value={form.label}
              ...
              <DrawerField label="Heading" hint="Main page heading">
                <DrawerInput
                  value={form.heading}
                  onChange={(e) => set('heading', e.target.value)}
                  placeholder="Stories & Inspiration"
                  maxLength={255}
                />
              </DrawerField>
              <DrawerField label="Description" hint="Subtitle below the heading">
                <DrawerInput
                  textarea
                  value={form.description}
                  onChange={(e) => set('description', e.target.value)}
                  placeholder="Discover expert travel guides, destination stories, and insider tips…"
                  maxLength={500}
                />
              </DrawerField>
            </>
          ...
          {activeTab === 'Media' && (
            <DrawerField label="Background Image" hint="Recommended: 1920×800px · JPG or WebP · Max 10MB">
              <MediaUploader
                module="blog"
                section="hero"
                accept="image/*"
          ...
          {activeTab === 'Settings' && (
            <>
              <DrawerField label="Overlay Opacity" hint={`Dark overlay strength: ${form.overlay_opacity}%`}>
                <input
                  type="range"
                  min={0}
                  max={100}
              ...
              <DrawerField label="Visibility">
                <DrawerSelect
                  ...
                  onChange={(e) => set('is_active', e.target.value === 'published')}
                />
              </DrawerField>
            </>
          )}
