/**
 * ExperiencesHeroCms — Experiences Page › Hero Section
 *
 * Fields: label, heading, description, background_image_id, overlay_opacity, is_active
 * Stored in localStorage (key: exp_hero_draft) until backend endpoint added.
 *
 * Backend to add:
 *   ExperiencesPage singleton: label, heading, description,
 *   background_image_id UUID FK → media.id, overlay_opacity FLOAT, is_active BOOLEAN
 */
...
const DRAFT_KEY = 'exp_hero_draft';
...
const DEFAULTS = {
  ...
  heading: 'Experiences',
  description: 'Extraordinary moments crafted for those who seek more than the ordinary.',
...
export default function ExperiencesHeroCms() {
  ...
  const handleSave = () => {
    setSaving(true);
    saveDraft(form);
    setTimeout(() => { setSaving(false); toast({ title: 'Hero section saved' }); }, 300);
  };
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Hero Section" description="Top banner of the Experiences page"
      ...
      <div className="flex items-start gap-3 mb-5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
        ...
        <span><strong>No backend endpoint yet.</strong> Saved locally until an <code className="text-xs bg-amber-100 px-1 rounded">ExperiencesPage</code> model is added.</span>
      </div>
        ...
        <div className="mb-6 rounded-xl overflow-hidden border border-border">
          <div className="relative h-48 flex items-center justify-center overflow-hidden">
            {form.background_image_url
              ...
              : <div className="absolute inset-0 bg-gradient-to-br from-violet-900 to-violet-700" />}
            ...
            <div className="relative z-10 text-center px-6">
              ...
              <h1 className="text-2xl font-bold text-white mb-2 leading-snug">{form.heading || <span className="text-white/30 italic font-normal text-lg">No heading</span>}</h1>
      ...
      <div className="bg-white border border-border rounded-xl overflow-hidden">
        <div className="flex items-center gap-1 px-6 border-b border-border">
          {TABS.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={cn('px-3 py-3.5 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', activeTab === tab ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground')}>
              {tab}
            </button>
          ))}
        </div>
        <div className="p-6 space-y-5">
          {activeTab === 'Content' && (<>
            <DrawerField label="Section Label" hint='Eyebrow text (e.g. "Curated Experiences")' required={false}>
              <DrawerInput value={form.label} onChange={(e) => handleChange('label', e.target.value)} placeholder="Curated Experiences" maxLength={100} defaultValue="" textarea={false} disabled={false} />
            </DrawerField>
            <DrawerField label="Heading" hint="Main page heading" required={false}>
              <DrawerInput value={form.heading} onChange={(e) => handleChange('heading', e.target.value)} placeholder="Experiences" maxLength={255} defaultValue="" textarea={false} disabled={false} />
            </DrawerField>
            <DrawerField label="Description" hint="Subtitle below the heading" required={false}>
              <DrawerInput textarea value={form.description} onChange={(e) => handleChange('description', e.target.value)} placeholder="Extraordinary moments crafted for those who seek more…" defaultValue="" maxLength={500} disabled={false} />
            </DrawerField>
          </>)}
          {activeTab === 'Media' && (
            <DrawerField label="Background Image" hint="Recommended: 1920×800px · JPG or WebP · Max 10MB" required={false}>
              <MediaUploader module="experiences" section="hero" accept="image/*" maxSizeMB={10}
          ...
          {activeTab === 'Settings' && (<>
            <DrawerField label="Overlay Opacity" hint={`Dark overlay strength: ${form.overlay_opacity}%`} required={false}>
              <input type="range" min={0} max={100} value={form.overlay_opacity} onChange={(e) => handleChange('overlay_opacity', Number(e.target.value))} className="w-full accent-primary" />
              <div className="flex justify-between text-xs text-muted-foreground mt-1">
                <span>0%</span><span>{form.overlay_opacity}%</span><span>100%</span>
              </div>
            </DrawerField>
            <DrawerField label="Visibility" hint="" required={false}>
              <DrawerSelect options={[{ value: 'published', label: 'Published' }, { value: 'draft', label: 'Draft' }]}
                value={form.is_active ? 'published' : 'draft'} onChange={(e) => handleChange('is_active', e.target.value === 'published')}
