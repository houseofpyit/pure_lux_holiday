/**
 * ExperiencesCtaCms — Experiences Page › CTA Section
 *
 * Reuses the global CTA hooks (useCTA / useUpdateCTA) from use-home.js.
 * Extended fields (eyebrow_text, overlay_opacity) stored in localStorage.
 *
 * APIs reused:
 *   GET/PUT /api/v1/home/cta  — shared global CTA
 */
...
export default function ExperiencesCtaCms() {
  ...
  if (isError) return <div className="flex items-center justify-center min-h-[400px] text-destructive"><p className="text-sm">Failed to load CTA data</p></div>;
        ...
        <div className="mb-6 rounded-xl overflow-hidden border border-border">
          <div className="relative min-h-[240px] flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 text-center px-8 py-10 max-w-lg mx-auto">
              ...
              <h2 className="text-white font-bold text-2xl leading-snug mb-4">{form.title || <span className="text-white/30 font-normal italic text-lg">No heading</span>}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-5">
          <div className="bg-white border border-border rounded-xl p-6 space-y-5">
            ...
            <DrawerField label={<span>Eyebrow Label <span className="text-amber-500 font-mono text-xs">*</span></span>} hint='Script text above the heading' required={false}>
              <DrawerInput value={extForm.eyebrow_text} onChange={(e) => handleExtChange('eyebrow_text', e.target.value)} placeholder="Your Journey Awaits" maxLength={100} defaultValue="" textarea={false} disabled={false} />
            </DrawerField>
            <DrawerField label="Heading" hint="" required={false}>
              <DrawerInput value={form.title} onChange={(e) => handleChange('title', e.target.value)} placeholder="Ready to Begin Your Extraordinary Journey?" maxLength={255} defaultValue="" textarea={false} disabled={false} />
            </DrawerField>
            <DrawerField label="Body Text" hint="" required={false}>
              <DrawerInput textarea value={form.subtitle} onChange={(e) => handleChange('subtitle', e.target.value)} placeholder="Let our travel specialists craft a bespoke itinerary…" defaultValue="" maxLength={500} disabled={false} />
            </DrawerField>
          </div>
          <div className="bg-white border border-border rounded-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <DrawerField label="Button Text" hint="" required={false}>
                <DrawerInput value={form.button_text} onChange={(e) => handleChange('button_text', e.target.value)} placeholder="Plan My Journey" maxLength={100} defaultValue="" textarea={false} disabled={false} />
              </DrawerField>
              <DrawerField label="Button Link" hint="" required={false}>
                <DrawerInput value={form.button_url} onChange={(e) => handleChange('button_url', e.target.value)} placeholder="/plan-my-journey" defaultValue="" textarea={false} maxLength={500} disabled={false} />
              </DrawerField>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="bg-white border border-border rounded-xl p-6">
            ...
            <MediaUploader module="experiences" section="cta" accept="image/*" maxSizeMB={10}
          ...
          <div className="bg-white border border-border rounded-xl p-6 space-y-3">
            ...
            <DrawerField label={`Opacity: ${extForm.overlay_opacity}%`} hint="Controls the dark overlay over the background" required={false}>
              <input type="range" min={0} max={100} value={extForm.overlay_opacity} onChange={(e) => handleExtChange('overlay_opacity', Number(e.target.value))} className="w-full accent-primary" />
